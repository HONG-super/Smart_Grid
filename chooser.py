def chooser(current_price, hold_charge, solar, load, E_max, Estored,expected_future_peak):
    surplus = solar - load
# free solar  when store it
    if surplus > 0 and Estored < E_max: # if there is a surplus and the energy stored is less than the max  then store
        charge = min(E_max - Estored, surplus)
        Estored = Estored + charge
        P_grid = -(surplus - charge)
...
# if there is a deficit, and room in batt
        ...
        deficit = load - solar
        discharge = min(deficit, Estored)
        Estored = Estored - discharge
        P_grid = deficit - discharge
