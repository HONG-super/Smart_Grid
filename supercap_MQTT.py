...
# Version: v29 + WiFi/MQTT interface
# MQTT additions:
# 1) Connects to broker 172.20.10.4:1884.
# 2) Subscribes to smartgrid/team01/supercap/command, system/emergency, system/restart.
# 3) Receives three states: charge/store, discharge/extract, maintain/hold.
# 4) Receives charge/discharge values as energy_j directly, or power_ref_w converted over 5 s.
# 5) Publishes voltage, current, power, soc, status to smartgrid/team01/supercap/telemetry.
# Control/PWM/current-loop logic is kept the same as the serial version.
...
MQTT_PORT = 1884
MQTT_CLIENT_ID = "team01_supercap"

TOPIC_SUPERCAP_COMMAND = b"smartgrid/team01/supercap/command"
TOPIC_SUPERCAP_TELEMETRY = b"smartgrid/team01/supercap/telemetry"
...
MQTT_TELEMETRY_INTERVAL_MS = 1000
WIFI_RETRY_MS = 5000
MQTT_RETRY_MS = 5000

# If backend sends only power_ref_w, this firmware converts it to an energy
# action using the existing 5-second trade window: energy_j = |power_ref_w| * 5 s.
# If backend sends energy_j, the value is used directly and no conversion is done.
POWER_REF_WINDOW_S = 5.0
...
CSV_LOG_TIME_MS = 80000      # 80 seconds
CSV_LOG_INTERVAL_MS = 50     # one row every 50 ms
...
# ============================================================
# Snapshot for P Command / MQTT Telemetry
# ============================================================
...
last_mqtt_service_ms = 0
last_mqtt_telemetry_ms = 0
mqtt_emergency_enabled = False
mqtt_last_command_source = "none"
...
def print_status():
    ...
    print(
        ...
        "Pcap={:.3f}W E={:.3f}J Espace={:.3f}J Eout={:.3f}J "
...
def mqtt_status_text():
    if mqtt_emergency_enabled:
        return "emergency"
...
def mqtt_hard_stop(reason):
    global trip, trip_reason, command
    global action_in_progress, mode, I_target
    global E_delta, E_target_action, store_deadline_reported
    global mqtt_emergency_enabled
    ...
    print("MQTT_HARD_STOP,{}".format(reason))


def queue_supercap_command(action, amount_j=None, source="mqtt"):
    global command, command_energy_j, mqtt_last_command_source
    global mqtt_emergency_enabled

    if mqtt_emergency_enabled and action not in ("H", "P", "C"):
        print("MQTT_COMMAND_BLOCKED_EMERGENCY,{}".format(source))
        return
    ...
    if action in ("S", "E"):
        if amount_j is None:
            ...
            amount = parse_energy_amount(str(amount_j))

        if amount is None:
            print("MQTT_BAD_ENERGY,{},{}".format(source, amount_j))
            return

        command_energy_j = amount
        command = action
        mqtt_last_command_source = source

        print(
            "MQTT_COMMAND_APPLIED,{},{} {:.3f}J".format(
                source,
                command,
                command_energy_j
            )
    ...
    if action in ("U", "H", "P", "C"):
        command = action
        mqtt_last_command_source = source
        print("MQTT_COMMAND_APPLIED,{},{}".format(source, command))
        return

    print("MQTT_UNKNOWN_ACTION,{},{}".format(source, action))


def queue_power_ref(power_w, source="power_ref_w"):
    try:
        power = float(power_w)
    except Exception as error:
        print("MQTT_BAD_POWER,{},{}".format(power_w, error))
        return

    if abs(power) < 0.001:
        queue_supercap_command("U", None, source + "=0W")
        return

    energy_j = abs(power) * POWER_REF_WINDOW_S

    if power > 0:
        queue_supercap_command("S", energy_j, source + "->S")
    else:
        queue_supercap_command("E", energy_j, source + "->E")

    print(
        "MQTT_POWER_TO_ENERGY,{:.3f}W over {:.3f}s -> {:.3f}J".format(
            power,
            POWER_REF_WINDOW_S,
            energy_j
        )
...
def handle_mqtt_mode(mode_text, data, source):
    mode_name = str(mode_text).strip().lower()

    energy_key, energy_value = find_first_key(
        data,
        (
            "energy_j",
            "target_energy_j",
    ...
    power_key, power_value = find_first_key(
        data,
        (
            "power_ref_w",
            "power_w",
    ...
    if mode_name in ("charge", "charging", "store", "store_energy", "s"):
        if energy_key is not None:
            queue_supercap_command("S", energy_value, source + ":" + energy_key)
        elif power_key is not None:
            queue_power_ref(abs(float(power_value)), source + ":" + power_key)
        else:
            queue_supercap_command("S", E_STEP, source + ":default")
        return

    if mode_name in ("discharge", "discharging", "extract", "extract_energy", "e"):
        if energy_key is not None:
            queue_supercap_command("E", energy_value, source + ":" + energy_key)
        elif power_key is not None:
            queue_power_ref(-abs(float(power_value)), source + ":" + power_key)
        else:
            queue_supercap_command("E", E_STEP, source + ":default")
        return

    if mode_name in ("maintain", "hold", "u", "idle_float"):
        queue_supercap_command("U", None, source + ":maintain")
        return

    if mode_name in ("stop", "stopped", "off", "idle", "h"):
        queue_supercap_command("H", None, source + ":stop")
        return

    print("MQTT_UNKNOWN_MODE,{},{}".format(source, mode_text))
...
def mqtt_callback(topic, msg):
    ...
    try:
        ...
        # Accept raw commands: S10, E5, U, H, or raw power number like 1.5 / -1.5.
        raw = msg_text.strip()
        upper = raw.upper()

        if upper.startswith("S"):
            queue_supercap_command("S", parse_energy_amount(upper[1:]), "raw_text")
            return

        if upper.startswith("E"):
            queue_supercap_command("E", parse_energy_amount(upper[1:]), "raw_text")
            return

        if upper in ("U", "MAINTAIN", "HOLD"):
            queue_supercap_command("U", None, "raw_text")
            return

        if upper in ("H", "STOP", "OFF"):
            queue_supercap_command("H", None, "raw_text")
            return

        try:
            queue_power_ref(float(raw), "raw_power")
            return
        except Exception:
            print("MQTT_RX_BAD_JSON,{},{}".format(topic_text, msg_text))
            return

    # JSON number: interpret as power_ref_w. Positive = charge, negative = discharge, zero = maintain.
    if isinstance(data, (int, float)):
        queue_power_ref(data, "raw_json_power")
        return

    # JSON string: can be S10 / E5 / U / H / numeric power.
    if isinstance(data, str):
        upper = data.strip().upper()
        if upper.startswith("S"):
            queue_supercap_command("S", parse_energy_amount(upper[1:]), "json_string")
            return
        if upper.startswith("E"):
            queue_supercap_command("E", parse_energy_amount(upper[1:]), "json_string")
            return
        if upper in ("U", "MAINTAIN", "HOLD"):
            queue_supercap_command("U", None, "json_string")
            return
        if upper in ("H", "STOP", "OFF"):
            queue_supercap_command("H", None, "json_string")
            return
        try:
            queue_power_ref(float(data), "json_string_power")
            return
        except Exception:
    ...
    command_text = str(data.get("command", "")).strip().lower()

    if topic == TOPIC_SYSTEM_EMERGENCY or command_text == "emergency_stop":
        enable = bool_value(data.get("enable", True), True)

        if enable:
            mqtt_emergency_enabled = True
            mqtt_hard_stop("emergency_stop")
    ...
    if topic == TOPIC_SYSTEM_RESTART or command_text == "restart":
        mqtt_emergency_enabled = False
        queue_supercap_command("H", None, "restart")
        print("MQTT_RESTART_RECEIVED")
        return

    # Direct command aliases.
    if command_text in ("stop", "disable", "off"):
        queue_supercap_command("H", None, "command:" + command_text)
        return

    if command_text in ("maintain", "hold"):
        queue_supercap_command("U", None, "command:" + command_text)
        return

    # set_mode form from the interface document.
    mode_key, mode_value = find_first_key(data, ("mode", "state", "target_state"))
    if command_text in ("set_mode", "set_state", "") and mode_key is not None:
        handle_mqtt_mode(mode_value, data, "mode")
        return

    # Direct energy command without explicit mode.
    energy_key, energy_value = find_first_key(
        data,
        ("energy_j", "target_energy_j", "amount_j", "energy", "value_j", "joules", "j")
    )

    if command_text in ("store", "charge", "s"):
        queue_supercap_command("S", energy_value if energy_key is not None else E_STEP, "command:" + command_text)
        return

    if command_text in ("extract", "discharge", "e"):
        queue_supercap_command("E", energy_value if energy_key is not None else E_STEP, "command:" + command_text)
        return

    # Direct power reference without explicit mode.
    # Positive power => STORE/charge, negative power => EXTRACT/discharge, zero => MAINTAIN.
    power_key, power_value = find_first_key(
        data,
        ("power_ref_w", "power_w", "target_power_w", "power", "value_w")
    )

    if power_key is not None:
        queue_power_ref(power_value, power_key)
        return

    # Direct action field.
    action_key, action_value = find_first_key(data, ("action", "cmd"))
    if action_key is not None:
        action_text = str(action_value).strip().lower()
        if action_text in ("s", "store", "charge"):
            queue_supercap_command("S", energy_value if energy_key is not None else E_STEP, action_key)
            return
        if action_text in ("e", "extract", "discharge"):
            queue_supercap_command("E", energy_value if energy_key is not None else E_STEP, action_key)
            return
        if action_text in ("u", "maintain", "hold"):
            queue_supercap_command("U", None, action_key)
            return
        if action_text in ("h", "stop", "off"):
            queue_supercap_command("H", None, action_key)
            return

    print("MQTT_RX_NO_COMMAND,{},{}".format(topic_text, msg_text))


def start_wifi_if_needed(now_ms):
    ...
    if (
        ...
        time.ticks_diff(now_ms, last_wifi_attempt_ms) >= WIFI_RETRY_MS
    ):
...
def connect_mqtt_if_needed(now_ms):
    ...
    if not start_wifi_if_needed(now_ms):
        return

    if (
        last_mqtt_attempt_ms != 0 and
        time.ticks_diff(now_ms, last_mqtt_attempt_ms) < MQTT_RETRY_MS
    ):
        return
    ...
    try:
        ...
        mqtt_client.subscribe(TOPIC_SUPERCAP_COMMAND)
        ...
        print("MQTT_SUB,{}".format(topic_to_str(TOPIC_SUPERCAP_COMMAND)))
...
def mqtt_publish_telemetry(now_ms):
    global mqtt_connected, mqtt_client
    ...
    try:
        payload = {
            "device": "supercap",
            "voltage": round(last_va, 3),
            "terminal_voltage": round(last_va_terminal, 3),
            "current": round(last_IL, 3),
            "power": round(last_power, 3),
            "soc": round(last_SoC, 1),
            "energy": round(last_E, 3),
            "energy_delta": round(E_delta, 3),
            "current_target": round(I_target, 3),
            "mode": mode,
            "status": mqtt_status_text(),
            "trip": bool(trip),
            "trip_reason": trip_reason,
            "duty": int(duty),
            "pwm_applied": int(last_pwm_applied),
            "timestamp_ms": int(now_ms)
        }

        payload_text = json.dumps(payload)
        mqtt_client.publish(TOPIC_SUPERCAP_TELEMETRY, payload_text.encode())
...
def mqtt_service(now_ms):
    global last_mqtt_service_ms, last_mqtt_telemetry_ms

    if time.ticks_diff(now_ms, last_mqtt_service_ms) >= MQTT_SERVICE_INTERVAL_MS:
        last_mqtt_service_ms = now_ms
        connect_mqtt_if_needed(now_ms)
        mqtt_check_messages()

    if time.ticks_diff(now_ms, last_mqtt_telemetry_ms) >= MQTT_TELEMETRY_INTERVAL_MS:
        last_mqtt_telemetry_ms = now_ms
        mqtt_publish_telemetry(now_ms)
...
print("Ready. v29 PID/PI current control + WiFi/MQTT supercap interface. Commands: S[J] E[J] U H P C")
print("Examples: S8 charges 8J, E3.5 extracts 3.5J. S/E alone use default.")
print("MQTT broker: {}:{}".format(MQTT_BROKER, MQTT_PORT))
print("MQTT command topic: {}".format(topic_to_str(TOPIC_SUPERCAP_COMMAND)))
print("MQTT telemetry topic: {}".format(topic_to_str(TOPIC_SUPERCAP_TELEMETRY)))
print("MQTT modes: charge/store, discharge/extract, maintain/hold. energy_j direct; power_ref_w converted using {:.1f}s window.".format(POWER_REF_WINDOW_S))
...
while True:
    ...
    # WiFi/MQTT service is outside the current controller.
    # It only converts backend messages into the same command variable used by serial input.
    mqtt_service(now_ms)
    ...
    if mqtt_emergency_enabled and command not in ("H", "P", "C"):
        if command:
            print("Emergency active - command ignored until system/restart or emergency enable=false.")
        command = ""
